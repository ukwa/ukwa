...
public class CollectionTree implements Serializable {
    ...
    public List<Target> getTargets(){
        return getTargets(true, null);
    }

    public List<Target> getTargets(boolean recursive, Pattern filterPattern){
        return Stream.concat(
                targets.stream()
                        // If there's a filter, filter on title:
                        .filter(t -> filterPattern == null || filterPattern.matcher(t.title).matches()),
                children.stream()
                        // Only include children if recursive is true
                        .filter(ct -> recursive)
                        .map(ct -> ct.getTargets(true, filterPattern))
                        .flatMap(t -> t.stream()))
